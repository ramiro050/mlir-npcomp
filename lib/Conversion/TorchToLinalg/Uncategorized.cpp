...
public:
  ...
  matchAndRewrite(AtenNllLossBackwardOp op, OpAdaptor adaptor,
    ...
    auto getAffineExprsForSingleElementTensor = [&](Value tensor) {
      auto tensorType = tensor.getType().cast<RankedTensorType>();
      return SmallVector<AffineExpr>(tensorType.getRank(),
                                     rewriter.getAffineConstantExpr(0));
    };

    SmallVector<AffineExpr> gradOutExpr;
    if (reduction == torch_upstream::Reduction::None && inputRank == 2)
      gradOutExpr = {rewriter.getAffineDimExpr(0)};
    else
      gradOutExpr = getAffineExprsForSingleElementTensor(gradOutput);

    SmallVector<AffineExpr> targetExpr;
    if (inputRank == 2)
      targetExpr = {rewriter.getAffineDimExpr(0)};
    else
      targetExpr = getAffineExprsForSingleElementTensor(target);

    SmallVector<AffineExpr>
        totalWeightExpr = getAffineExprsForSingleElementTensor(totalWeight);

    SmallVector<AffineExpr> resultExpr{rewriter.getAffineDimExpr(0)};
    if (inputRank == 2)
      resultExpr.push_back(rewriter.getAffineDimExpr(1));
    SmallVector<StringRef> iteratorTypes(inputRank,
                                         getParallelIteratorTypeName());
    auto indexingMaps = AffineMap::inferFromExprList(
        {gradOutExpr, targetExpr, totalWeightExpr, resultExpr});
